...
namespace CarrotFantasy
{
    ...
    public class BattleFlowFieldComponent : BaseBattleComponent
    {
        ...
        /// <summary>
        /// 生存模式 PVE 刷怪由 <see cref="BattleSurvivalPVEMonsterComponent"/> 使用 <see cref="BattleUnit_MonsterFlow"/>。
        /// 保留字段便于后续在流场战斗内做移动模式开关。
        /// </summary>
        public bool useMonsterFlowMovement = true;

        private BattlePVEMapComponent map;
        private int xc;
        private int yr;
        ...
        {
            this.map = BattlePVEMapComponent.GetFrom(this.baseBattle);
            if (this.map == null)
            {
                this.staticWalkable = new bool[0];
                this.IsBuilt = false;
                return;
            }
            ...
            List<BattleMapGrid.GridIndex> path = this.map.LevelInfo != null ? this.map.LevelInfo.monsterPath : null;
