...
namespace CarrotFantasy
{
    public class BattleItemComponent : BaseBattleComponent
    {
        ...
        public override void Init()
        {
            ...
            this.dataComponent = (BattlePVEDataComponent)this.baseBattle.GetComponent(BattleComponentType.DataComponent);
            BattleMapGrid[,] gridsList = mapComponent.gridsList;

            for (int x = 0; x <= dataComponent.xColumn - 1; x++)
            {
                for (int y = 0; y <= dataComponent.yRow - 1; y++)
                {
